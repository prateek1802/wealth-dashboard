...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (!userId) {
      return NextResponse.json({ ok: true, updated: 0, skipped: [], note: "No assets exist yet — nothing to refresh." });
    }
    ...
    for (const holding of holdings) {
      ...
      // explicit userId below (price_history.upsert()). Checked, not
      // assumed — this route makes exactly these two writes, nothing else.
